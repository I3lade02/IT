...
document.write(secti(1,2) + "<br>");
document.write(secti(6, 2) + "<br>");


let parts = ['ramena', 'kolena'];
let lyrics = ['Hlava', ...parts, 'a', 'Palce'];

lyrics.forEach(s => document.write(s + ", "));

document.write("<br>");

function naplnPole(cisla, delkaPole) {
    for (let i = 0; i < delkaPole; i++){
        cisla[i] = parseInt(Math.round(Math.random() * 10));
    }
}

const poleCisel = [];
naplnPole(poleCisel, 50);

poleCisel.forEach(c => document.write(c + ", "));


let nadpis = document.getElementById("nadpis");
nadpis.innerHTML = "<p>Ahoj světe</p>";

let seznam = document.createElement("ul");
function vytvorSeznam() {
    for (let i = 0; i < 5; i++) {
        let polozka = document.createElement("li");
        polozka.innerText = "Hello"
        seznam.appendChidl(polozka);
    }
...
tlacitko.onclick = () => {
    const heslo = "paklíč";
    if (input.value ==  heslo) {
        window.location.replace("./tajne.html");
    } else {
        alert("špatné heslo");
    }
}

tlacitko.addEventListener("click", vstoupit());

let prepinac = document.getElementById("prepinac");

function prepni() {
    if (prepinac.getAttribute("class") == "btn btn-success btn-lg") {
        prepinac.setAttribute("class", "btn btn-danger btn-lg");
    } else {
        prepinac.setAttribute("class", "btn btn-success btn-lg");
    }
}

prepinac.addEventListener("click", prepni);